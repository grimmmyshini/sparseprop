...
class SparseConvFunction(torch.autograd.Function):
    ...
    def forward(
        ...
        if vectorizing_fwd_over_on:
            ...
            sppb.sparse_conv2d_vectorized_forward_over_on_stride_1(
                input, *W_idx, W_val, output, K, padding
            )
        else:
            input = input.permute(1, 2, 3, 0).contiguous()
            output = torch.zeros(OC, OM, ON, B).float()
            if stride == 1:
                sppb.sparse_conv2d_vectorized_forward_stride_1(
                    input, *W_idx, W_val, output, K, padding
                )
            elif stride == 2:
                sppb.sparse_conv2d_vectorized_forward_stride_2(
                    input, *W_idx, W_val, output, K, padding
                )
    ...
    def backward(ctx, grad_output):
        ...
        if vectorizing_bwd_over_on:
            if stride == 1:
                sppb.sparse_conv2d_vectorized_backward_over_on_stride_1(
                    input,
                    W_idx_OC,
                    W_idx_IC,
                    W_idx_X,
                ...
                sppb.sparse_conv2d_vectorized_backward_over_on_stride_2(
                    input,
                    W_idx_OC,
                    W_idx_IC,
                    W_idx_X,
            ...
            if stride == 1:
                sppb.sparse_conv2d_vectorized_backward_stride_1(
                    input,
                    W_idx_OC,
                    W_idx_IC,
                    W_idx_X,
                ...
                sppb.sparse_conv2d_vectorized_backward_stride_2(
                    input,
                    W_idx_OC,
                    W_idx_IC,
                    W_idx_X,
